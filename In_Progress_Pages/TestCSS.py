...
def edit_excel_template(data, template_path, num_orgs, num_years):
    ...
    for entry in data:
        for individual in entry.get('individuals_data', []):
            ...
            sheet[f"A{start_row_990}"] = index_counter
            ...
            sheet4[f"A{start_row_990pf}"] = index_counter
...
if eins_input:
    ...
    if st.button("Generate Final Output Chart"):
        ...
        num_unique_entries = len({(entry["EIN"], year) for entry in st.session_state['final_chart_data'] for year in range(num_years)})
        if final_chart_data:
